...
../gcc-src/configure --enable-languages=c,ymir --with-gcc-major-version-only --program-suffix=-${GCC_MAJOR_VERSION} --prefix=/usr --program-prefix=x86_64-linux-gnu- --libexecdir=/usr/lib --libdir=/usr/lib --with-sysroot=/ --with-arch-directory=amd64 --enable-multiarch --with-arch-32=i686 --with-abi=m64 --with-multilib-list=m32,m64,mx32 --enable-multilib --enable-checking=release --build=x86_64-linux-gnu --host=x86_64-linux-gnu --target=x86_64-linux-gnu --disable-bootstrap
...
Depends: g++-${GCC_MAJOR_VERSION} (>= $GCC_MAJOR_VERSION), gcc-${GCC_MAJOR_VERSION} (>= $GCC_MAJOR_VERSION)
EOF

dpkg-deb --build ${INSTALL_DIR}/gyc-${GCC_VERSION}-build/gcc-deb
mv ${INSTALL_DIR}/gyc-${GCC_VERSION}-build/gcc-deb.deb ${INSTALL_DIR}/gyc-${GCC_VERSION}-build/gyc-${GCC_VERSION}_amd64.deb
