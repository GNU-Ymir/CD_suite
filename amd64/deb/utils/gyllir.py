...
class GyllirSelfBuilder:
    """Builds gyllir >= 1.2.0, which compiles itself with a previously-released gyllir instead
    of CMake (jobs/gyllir_self_build, see GyllirSpec.prev_gyllir)."""
    ...
    def buildGyllir(self):
        shutil.copy (f"results/gyc-{self.gyc}_amd64.deb", "jobs/gyllir_self_build/gyc.deb")
        shutil.copy (f"results/gyllir_{self.prev_gyllir}_amd64.deb", "jobs/gyllir_self_build/gyllir.deb")
        generator = self.api.build(
            ...
            buildargs={
                ...
                "UBUNTU_VERSION": self.ubuntu_version,
                "GYLLIR_VERSION": self.gyllir_version,
                "GYLLIR_VERSION_NUMBER": self.gyllir_version,
                "ARCH": "amd64"
            }
        )

        self.showLogs(generator)
        os.remove ("jobs/gyllir_self_build/gyc.deb")
        os.remove ("jobs/gyllir_self_build/gyllir.deb")
