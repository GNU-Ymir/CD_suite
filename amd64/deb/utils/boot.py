...
class BootstrapBuilder:
    ...
    def _createFirstDebFile (self):
        ...
        self._vm.runCmd (f"sudo dpkg -r gyc-{self._gcc_major_version}") # remove previous version

        # Install the new one
        self._vm.runCmd (f"cd gcc/ && sudo dpkg -i gcc-bin.deb")
    ...
    def _buildMidgard (self):
        ...
        self._vm.runCmd ("cd gcc/midgard/.build && touch __lib_test_.o")
        self._vm.runCmd ("cd gcc/midgard/.build && touch __lib_debug_.o")
        self._vm.runCmd ("cd gcc/midgard/.build && touch __lib_release_.o")
